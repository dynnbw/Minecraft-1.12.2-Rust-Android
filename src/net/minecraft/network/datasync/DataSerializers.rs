...
use crate::net::minecraft::network::PacketBuffer::{
    read_bool, read_f32_be, read_i64_be, read_nbt_compound, read_string, read_u8,
    read_uuid, read_var_i32, CodecError,
};
...
pub fn readValue(serializerId: i32, input: &mut &[u8]) -> Result<DataValue, CodecError> {
    Ok(match serializerId {
        ...
        4 => {
            ...
            DataValue::TextComponent(ITextComponent::fromJsonLenient(&json)
        ...
        7 => DataValue::Rotations([read_f32_be(input)?, read_f32_be(input)?, read_f32_be(input)?]),
        ...
        11 => DataValue::OptionalUuid(if read_bool(input)? { Some(read_uuid(input)?) } else { None }),
        12 => {
        ...
        id => return Err(CodecError::InvalidData(format!("unknown entity data serializer {id}"))),
    })
}

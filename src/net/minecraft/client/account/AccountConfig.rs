...
impl AccountConfig {
    ...
    pub fn get(&self, index: usize) -> Option<&Account> { self.accounts.get(index) }
    pub fn iter(&self) -> impl ExactSizeIterator<Item = &Account> { self.accounts.iter() }
    ...
    pub fn replace(&mut self, index: usize, account: Account) -> io::Result<()> {
        let Some(slot) = self.accounts.get_mut(index) else {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "account index out of bounds"));
        };
        *slot = account;
        self.save()
    }
    ...
    pub fn remove(&mut self, index: usize) -> io::Result<Option<Account>> {
        if index >= self.accounts.len() { return Ok(None); }
    ...
    pub fn save(&self) -> io::Result<()> {
        ...
        let bytes = serde_json::to_vec_pretty(&StoredAccounts { accounts: self.accounts.clone() })
...
mod tests {
    ...
    fn exhibition_schema_round_trips() {
        let root = std::env::temp_dir().join(format!("mc112-account-test-{}", current_time_millis()));
        let mut config = AccountConfig::load(&root);
        config.add(Account::new("refresh", "access", "Player", 123, "uuid")).unwrap();

...
pub enum SkinVariant { Classic, Slim }
impl SkinVariant { pub const fn as_str(self) -> &'static str { match self { Self::Classic => "classic", Self::Slim => "slim" } } }
...
pub fn upload_skin(path: &Path, variant: SkinVariant, accessToken: &str) -> Result<(), SkinUploadError> {
    if accessToken.trim().is_empty() { return Err(SkinUploadError::MissingToken); }
    let bytes = fs::read(path).map_err(SkinUploadError::Read)?;
    if bytes.len() < 8 || &bytes[..8] != b"\x89PNG\r\n\x1a\n" { return Err(SkinUploadError::InvalidPng); }
    let boundary: String = rand::thread_rng().sample_iter(&Alphanumeric).take(24).map(char::from).collect();
    let filename = path.file_name().and_then(|value| value.to_str()).unwrap_or("skin.png");
        ...
        .send_bytes(&body)
    {
        ...
        Err(ureq::Error::Status(status, response)) => {
            ...
            Err(SkinUploadError::Http(if body.trim().is_empty() { format!("HTTP {status}") } else { format!("HTTP {status}: {}", body.trim()) }))
        }

...
impl ServerAddress {
    fn new(address: impl Into<String>, port: u16) -> Self { Self { ipAddress: address.into(), serverPort: port } }
    pub fn getIP(&self) -> String {
        if self.ipAddress.parse::<IpAddr>().is_ok() { return self.ipAddress.clone(); }
        idna::domain_to_ascii(&self.ipAddress).unwrap_or_default()
    }
    pub const fn getPort(&self) -> u16 { self.serverPort }
    ...
    pub fn fromString(addrString: &str) -> Self {
        ...
        let port = portText.and_then(|value| value.trim().parse::<u16>().ok()).unwrap_or(25565);
...
mod tests {
    use super::*;
    #[test] fn parses_vanilla_address_forms() {
        assert_eq!(ServerAddress::fromString("example.org").getPort(), 25565);
        assert_eq!(ServerAddress::fromString("example.org:25570").getPort(), 25570);
        assert_eq!(ServerAddress::fromString("[::1]:25566").getIP(), "::1");
        assert_eq!(ServerAddress::fromString("2001:db8::1").getIP(), "2001:db8::1");
    }
}

...
impl ModelGuardian {
    ...
    const SPINE_X: [f32; 12] = [0.0, 0.0, 8.0, -8.0, -8.0, 8.0, 8.0, -8.0, 0.0, 0.0, 8.0, -8.0];
    const SPINE_Y: [f32; 12] = [-8.0, -8.0, -8.0, -8.0, 0.0, 0.0, 0.0, 0.0, 8.0, 8.0, 8.0, 8.0];
    const SPINE_Z: [f32; 12] = [8.0, -8.0, 0.0, 0.0, -8.0, -8.0, 8.0, 8.0, 8.0, -8.0, 0.0, 0.0];
    ...
    pub fn boxes(input: LivingRenderInput, state: GuardianModelState) -> Vec<LivingModelBox> {
        ...
        boxes.push(model_box([0, 0], [-6.0, 10.0, -8.0], [12, 12, 16], 0.0, false, bodyPose, LivingModelGroup::Body));
        boxes.push(model_box([0, 28], [-8.0, 10.0, -6.0], [2, 12, 12], 0.0, false, bodyPose, LivingModelGroup::Body));
        boxes.push(model_box([0, 28], [6.0, 10.0, -6.0], [2, 12, 12], 0.0, true, bodyPose, LivingModelGroup::Body));
        boxes.push(model_box([16, 40], [-6.0, 8.0, -6.0], [12, 2, 12], 0.0, false, bodyPose, LivingModelGroup::Body));
        boxes.push(model_box([16, 40], [-6.0, 22.0, -6.0], [12, 2, 12], 0.0, false, bodyPose, LivingModelGroup::Body));
        ...
        let mut eye = model_box(
            ...
            0.0,
            false,
            PartPose { pivot: [eyeX, eyeY, -8.25], rotation: [0.0; 3] },
            LivingModelGroup::Body,
        );
        ...
        let tail0Pose = PartPose {
            pivot: [0.0; 3],
            rotation: [0.0, state.tailAnimation.sin() * std::f32::consts::PI * 0.05, 0.0],
        };
        let tail1Pose = PartPose {
            pivot: [-1.5, 0.5, 14.0],
            rotation: [0.0, state.tailAnimation.sin() * std::f32::consts::PI * 0.1, 0.0],
        };
        let tail2Pose = PartPose {
            pivot: [0.5, 0.5, 6.0],
            rotation: [0.0, state.tailAnimation.sin() * std::f32::consts::PI * 0.15, 0.0],
        };

        let mut tail0 = model_box([40, 0], [-2.0, 14.0, 7.0], [4, 4, 8], 0.0, false, tail0Pose, LivingModelGroup::Body);
        tail0.parentPose = Some(bodyPose);
        boxes.push(tail0);

        let mut tail1 = model_box([0, 54], [0.0, 14.0, 0.0], [3, 3, 7], 0.0, false, tail1Pose, LivingModelGroup::Body);
        ...
        for (texture, origin, size) in [
            ...
            let mut tail2 = model_box(texture, origin, size, 0.0, false, tail2Pose, LivingModelGroup::Body);
    ...
    fn eyeOffsets(state: GuardianModelState) -> (f32, f32) {
        let Some(focus) = state.focusEyes else { return (0.0, 0.0); };
        let eyeY = if focus[1] - state.guardianEyes[1] > 0.0 { 0.0 } else { 1.0 };
        let dx = state.guardianEyes[0] - focus[0];
        let dz = state.guardianEyes[2] - focus[2];
        let length = (dx * dx + dz * dz).sqrt();
        if length <= 1.0e-12 { return (0.0, eyeY); }
        // Vec3d.normalize().rotateYaw(PI/2): (x,z) -> (z,-x).
        let rotatedX = dz / length;
        let rotatedZ = -dx / length;
        let dot = (state.guardianLook[0] * rotatedX
            + state.guardianLook[2] * rotatedZ) as f32;
...
mod tests {
    ...
    fn model_contains_body_spines_eye_and_tail_boxes() {
        let boxes = ModelGuardian::boxes(input(), GuardianModelState {
            spikesAnimation: 1.0, tailAnimation: 0.0,
            guardianEyes: [0.0, 0.0, 0.0], focusEyes: None,
    ...
    fn fully_extended_first_spine_uses_mcp_position() {
        let boxes = ModelGuardian::boxes(input(), GuardianModelState {
            spikesAnimation: 1.0, tailAnimation: 0.0,
            guardianEyes: [0.0; 3], focusEyes: None, guardianLook: [0.0, 0.0, 1.0],
        });

...
impl ModelQuadruped {
    pub fn pose(input: LivingRenderInput, height: i32) -> QuadrupedPose {
        ...
        QuadrupedPose {
            ...
            leg1: leg_pose([-3.0, (24 - height) as f32, 7.0], phase.cos() * 1.4 * input.limbSwingAmount),
            leg2: leg_pose([3.0, (24 - height) as f32, 7.0], (phase + std::f32::consts::PI).cos() * 1.4 * input.limbSwingAmount),
            leg3: leg_pose([-3.0, (24 - height) as f32, -5.0], (phase + std::f32::consts::PI).cos() * 1.4 * input.limbSwingAmount),
            leg4: leg_pose([3.0, (24 - height) as f32, -5.0], phase.cos() * 1.4 * input.limbSwingAmount),
        }
    }

    pub fn boxes(pose: QuadrupedPose, height: i32, scale: f32) -> Vec<LivingModelBox> {
        vec![
            model_box([0, 0], [-4.0, -4.0, -8.0], [8, 8, 8], scale, false, pose.head, LivingModelGroup::Head),
            model_box([28, 8], [-5.0, -10.0, -7.0], [10, 16, 8], scale, false, pose.body, LivingModelGroup::Body),
            model_box([0, 16], [-2.0, 0.0, -2.0], [4, height, 4], scale, false, pose.leg1, LivingModelGroup::Body),
            model_box([0, 16], [-2.0, 0.0, -2.0], [4, height, 4], scale, false, pose.leg2, LivingModelGroup::Body),
            model_box([0, 16], [-2.0, 0.0, -2.0], [4, height, 4], scale, false, pose.leg3, LivingModelGroup::Body),
            model_box([0, 16], [-2.0, 0.0, -2.0], [4, height, 4], scale, false, pose.leg4, LivingModelGroup::Body),
        ]
    }
...
const fn leg_pose(pivot: [f32; 3], rotateX: f32) -> PartPose {
    PartPose { pivot, rotation: [rotateX, 0.0, 0.0] }
}
...
mod tests {
    ...
    fn input() -> LivingRenderInput {
        LivingRenderInput {
            ...
            limbSwing: 0.0, limbSwingAmount: 1.0, ageInTicks: 0.0,

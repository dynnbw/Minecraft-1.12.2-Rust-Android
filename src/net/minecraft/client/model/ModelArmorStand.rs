...
impl ModelArmorStand {
    ...
    pub fn pose(
        ...
        let biped = ModelArmorStandArmor::pose(
            head, body, leftArm, rightArm, leftLeg, rightLeg,
        );
        let supportPose = PartPose { pivot: [0.0, 0.0, 0.0], rotation: biped.body.rotation };
        ArmorStandPose {
            biped,
    ...
    pub fn boxes(pose: ArmorStandPose, delta: f32) -> Vec<LivingModelBox> {
        let mut boxes = vec![
            model_box([0, 0], [-1.0, -7.0, -1.0], [2, 7, 2], delta, false, pose.biped.head, LivingModelGroup::Head),
            model_box([0, 26], [-6.0, 0.0, -1.5], [12, 3, 3], delta, false, pose.biped.body, LivingModelGroup::Body),
            model_box([8, 0], [-1.0, 0.0, -1.0], [2, 11, 2], delta, false, pose.biped.rightLeg, LivingModelGroup::Body),
            model_box([40, 16], [-1.0, 0.0, -1.0], [2, 11, 2], delta, true, pose.biped.leftLeg, LivingModelGroup::Body),
            model_box([16, 0], [-3.0, 3.0, -1.0], [2, 7, 2], delta, false, pose.standRightSide, LivingModelGroup::Body),
            model_box([48, 16], [1.0, 3.0, -1.0], [2, 7, 2], delta, false, pose.standLeftSide, LivingModelGroup::Body),
            model_box([0, 48], [-4.0, 10.0, -1.0], [8, 2, 2], delta, false, pose.standWaist, LivingModelGroup::Body),
        ];
        if pose.showArms {
            boxes.push(model_box([24, 0], [-2.0, -2.0, -1.0], [2, 12, 2], delta, false, pose.biped.rightArm, LivingModelGroup::Body));
            boxes.push(model_box([32, 16], [0.0, -2.0, -1.0], [2, 12, 2], delta, true, pose.biped.leftArm, LivingModelGroup::Body));
        }
        if pose.showBase {
            boxes.push(model_box([0, 32], [-6.0, 11.0, -6.0], [12, 1, 12], delta, false, pose.standBase, LivingModelGroup::Body));
        }
        boxes
    }
}
...
mod tests {
    ...
    fn marker_does_not_hide_the_visible_model() {
        let pose = ModelArmorStand::pose(
            input(), [0.0; 3], [0.0; 3], [-10.0, 0.0, -10.0],
            [-15.0, 0.0, 10.0], [-1.0, 0.0, -1.0], [1.0, 0.0, 1.0],
            false, false, true,
        );

...
impl SoundManager {
    pub fn new() -> Self { Self::withBackend(createPlatformBackend()) }
    ...
    pub const fn isLoaded(&self) -> bool { self.loaded }
    pub const fn getPlayTime(&self) -> i32 { self.playTime }

    pub fn reloadSoundSystem(&mut self, soundLevels: &[f32; 10]) {
        self.stopAllSounds();
        self.loaded = self.backend.isLoaded();
        self.backend.setMasterVolume(soundLevels[SoundCategory::Master.index()]);
    }
    ...
    pub fn playResolvedSound(
        ...
        if channelVolume <= 0.0 { return None; }
        ...
        if let Err(error) = self.backend.play(request) {
            log::warn!("Unable to start sound {}: {error}", selected.getSoundLocation());
            return None;
        }

        self.playingSounds.insert(channel, PlayingSound {
            record,
            selected,
    ...
    pub fn playDelayedSound(&mut self, sound: PositionedSoundRecord, delay: i32) {
        self.delayedSounds.push((sound, self.playTime.saturating_add(delay.max(0))));
    }
    ...
    pub fn updateAllSounds(&mut self, _soundLevels: &[f32; 10]) -> Vec<PositionedSoundRecord> {
        ...
        let ended = self.playingSounds.iter()
        ...
        for channel in ended {
            let Some(sound) = self.playingSounds.remove(&channel) else { continue; };
            self.backend.remove(channel);
            if sound.record.repeat && sound.record.repeatDelay > 0 {
                let repeatDelay = sound.record.repeatDelay;
                self.delayedSounds.push((
                    sound.record,
                    self.playTime.saturating_add(repeatDelay),
                ));
            }
            self.pausedChannels.retain(|candidate| *candidate != channel);
        }
    ...
    pub fn stopRecordAt(&mut self, position: [f32; 3]) {
        let channels = self.playingSounds.iter().filter_map(|(&channel, sound)| {
    ...
    pub fn isChannelPlaying(&self, channel: u64) -> bool {
        self.playingSounds.get(&channel).is_some_and(|sound| {
            self.backend.isPlaying(channel) || sound.stopTime > self.playTime
        })
    }
    ...
    pub fn resumeAllSounds(&mut self) {
        for channel in self.pausedChannels.drain(..) { self.backend.resume(channel); }
    }
    ...
    pub fn setVolume(&mut self, category: SoundCategory, soundLevels: &[f32; 10]) {
        if category == SoundCategory::Master {
            ...
            if soundLevels[category.index()] <= 0.0 { self.stopAllSounds(); }
            return;
        }

        let updates = self.playingSounds.iter().map(|(&channel, playing)| {
            let resolved = playing.record.volume * playing.selected.getVolume();
            (channel, clamped_category_volume(playing.record.category, resolved, soundLevels))
        }).collect::<Vec<_>>();
    ...
    pub fn stop(
        ...
        let channels = self.playingSounds.iter().filter_map(|(&channel, sound)| {
            ...
            let eventMatches = event.map_or(true, |value| value == &sound.record.positionedSoundLocation);
    ...
    pub fn setListener(
        ...
        self.backend.setListener(ListenerTransform { position, forward, up });
    }
    ...
    pub fn setSoundVolumePitch(
        ...
        let Some(sound) = self.playingSounds.get_mut(&channel) else { return; };
        ...
        let channelVolume = clamped_category_volume(sound.record.category, resolvedVolume, soundLevels);
        self.backend.setVolume(channel, channelVolume);
...
mod tests {
    ...
    impl SoundBackend for MockBackend {
        ...
        fn isPlaying(&self, channel: u64) -> bool { self.0.lock().unwrap().playing.contains(&channel) }
        fn stop(&mut self, channel: u64) {
            let mut state = self.0.lock().unwrap();
            state.playing.remove(&channel);
            state.stopped.push(channel);
        }
        fn remove(&mut self, channel: u64) { self.0.lock().unwrap().playing.remove(&channel); }
        fn stopAll(&mut self) { self.0.lock().unwrap().playing.clear(); }
    ...
    fn delayed_repeat_returns_event_for_fresh_resolution() {
        ...
        let channel = manager.playResolvedSound(record(true, 3), selected(), Vec::new(), &[1.0; 10]).unwrap();
        state.lock().unwrap().playing.remove(&channel);
        for _ in 0..20 { assert!(manager.updateAllSounds(&[1.0; 10]).is_empty()); }
        for _ in 0..2 { assert!(manager.updateAllSounds(&[1.0; 10]).is_empty()); }

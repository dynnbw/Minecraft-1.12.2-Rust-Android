...
impl SoundList {
    pub fn fromJson(value: &Value) -> Result<Self, String> {
        let object = value.as_object().ok_or_else(|| "sound event entry must be an object".to_owned())?;
        let replaceExisting = object.get("replace").and_then(Value::as_bool).unwrap_or(false);
        ...
        if let Some(entries) = object.get("sounds") {
            let entries = entries.as_array().ok_or_else(|| "sound event sounds must be an array".to_owned())?;
            for entry in entries {
                sounds.push(parse_sound(entry)?);
            }
        }
        Ok(Self { sounds, replaceExisting, subtitle })
    }

    pub fn getSounds(&self) -> &[Sound] { &self.sounds }
    pub const fn canReplaceExisting(&self) -> bool { self.replaceExisting }
...
fn parse_sound(value: &Value) -> Result<Sound, String> {
    ...
    let object = value.as_object().ok_or_else(|| "sound entry must be a string or object".to_owned())?;
    let name = object.get("name").and_then(Value::as_str)
        .ok_or_else(|| "sound object requires a string name".to_owned())?;
    let volume = number(object.get("volume"), 1.0, "volume")?;
    if volume <= 0.0 { return Err("Invalid volume".to_owned()); }
    let pitch = number(object.get("pitch"), 1.0, "pitch")?;
    if pitch <= 0.0 { return Err("Invalid pitch".to_owned()); }
    ...
    if weight <= 0 { return Err("Invalid weight".to_owned()); }
    let streaming = match object.get("stream") {
        None => false,
        Some(value) => value.as_bool().ok_or_else(|| "sound stream must be a boolean".to_owned())?,
    };
    let soundType = match object.get("type") {
        None => Type::File,
        Some(value) => Type::getByName(value.as_str().ok_or_else(|| "sound type must be a string".to_owned())?)
            .ok_or_else(|| "Invalid type".to_owned())?,
    };
    Ok(Sound::new(name, volume, pitch, weight, soundType, streaming))
}

fn number(value: Option<&Value>, default: f32, field: &str) -> Result<f32, String> {
    match value {
        None => Ok(default),
        Some(value) => value.as_f64().map(|value| value as f32)

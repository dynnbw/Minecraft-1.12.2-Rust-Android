...
impl ParticleEndRod {
    pub fn new(x:f64,y:f64,z:f64,xSpeed:f64,ySpeed:f64,zSpeed:f64,random:&mut impl Rng)->Self{
        ...
        p.motionX=xSpeed;p.motionY=ySpeed;p.motionZ=zSpeed;
        p.particleScale*=0.75;
        p.particleMaxAge=60+random.gen_range(0..12);
        Self{particle:p,fadeTarget:[242.0/255.0,222.0/255.0,201.0/255.0]}
    }
    pub fn onUpdate(&mut self,world:&WorldClient){
        ...
        if p.particleAge>p.particleMaxAge/2 {
            p.setAlphaF(1.0-(p.particleAge as f32-(p.particleMaxAge/2) as f32)/p.particleMaxAge.max(1) as f32);
        ...
        p.motionX*=0.91;p.motionY*=0.91;p.motionZ*=0.91;
    }
}

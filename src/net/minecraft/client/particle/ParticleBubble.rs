...
#[derive(Debug, Clone)]
pub struct ParticleBubble { pub particle: Particle }
...
impl ParticleBubble {
    pub fn new(x: f64, y: f64, z: f64, xSpeed: f64, ySpeed: f64, zSpeed: f64, random: &mut impl Rng) -> Self {
        ...
        particle.motionX = xSpeed * 0.20000000298023224 + (random.gen::<f64>() * 2.0 - 1.0) * 0.019999999552965164;
        particle.motionY = ySpeed * 0.20000000298023224 + (random.gen::<f64>() * 2.0 - 1.0) * 0.019999999552965164;
        particle.motionZ = zSpeed * 0.20000000298023224 + (random.gen::<f64>() * 2.0 - 1.0) * 0.019999999552965164;
    ...
    pub fn onUpdate(&mut self, world: &WorldClient) {
        ...
        let blockId = world.getBlockState(crate::net::minecraft::util::math::BlockPos::BlockPos::new(
            p.posX.floor() as i32, p.posY.floor() as i32, p.posZ.floor() as i32,
        )).getBlockId();
        if !matches!(blockId, 8 | 9) { p.isExpired = true; }
        let expired = p.particleMaxAge <= 0;
        p.particleMaxAge -= 1;
        if expired { p.isExpired = true; }
    }
}

#[path = "Gui.rs"] pub mod Gui;
#[path = "GuiScreen.rs"] pub mod GuiScreen;
#[path = "FontRenderer.rs"] pub mod FontRenderer;
#[path = "GuiButton.rs"] pub mod GuiButton;
#[path = "GuiOptionSlider.rs"] pub mod GuiOptionSlider;
#[path = "GuiMainMenu.rs"] pub mod GuiMainMenu;
#[path = "GuiButtonLanguage.rs"] pub mod GuiButtonLanguage;
#[path = "ScaledResolution.rs"] pub mod ScaledResolution;
#[path = "GuiOptions.rs"] pub mod GuiOptions;
#[path = "GuiVideoSettings.rs"] pub mod GuiVideoSettings;
#[path = "GuiMultiplayer.rs"] pub mod GuiMultiplayer;
#[path = "GuiWorldSelection.rs"] pub mod GuiWorldSelection;
#[path = "GuiLanguage.rs"] pub mod GuiLanguage;

#[path = "GuiTextField.rs"] pub mod GuiTextField;

#[path = "GuiScreenAddServer.rs"] pub mod GuiScreenAddServer;

#[path = "GuiScreenServerList.rs"] pub mod GuiScreenServerList;

#[path = "GuiYesNo.rs"] pub mod GuiYesNo;
#[path = "GuiDisconnected.rs"] pub mod GuiDisconnected;
#[path = "GuiDownloadTerrain.rs"] pub mod GuiDownloadTerrain;

#[path = "GuiIngame.rs"] pub mod GuiIngame;
#[path = "GuiIngameMenu.rs"] pub mod GuiIngameMenu;
#[path = "GuiGameOver.rs"] pub mod GuiGameOver;
#[path = "GuiWinGame.rs"] pub mod GuiWinGame;

#[path = "GuiRepair.rs"] pub mod GuiRepair;
#[path = "GuiEnchantment.rs"] pub mod GuiEnchantment;
#[path = "GuiMerchant.rs"] pub mod GuiMerchant;
#[path = "GuiHopper.rs"] pub mod GuiHopper;
#[path = "inventory/mod.rs"] pub mod inventory;

#[path = "GuiPlayerTabOverlay.rs"] pub mod GuiPlayerTabOverlay;

#[path = "ChatLine.rs"] pub mod ChatLine;
#[path = "GuiNewChat.rs"] pub mod GuiNewChat;
#[path = "GuiChat.rs"] pub mod GuiChat;

#[path = "BossInfoClient.rs"] pub mod BossInfoClient;

#[path = "GuiBossOverlay.rs"] pub mod GuiBossOverlay;

#[path = "MapItemRenderer.rs"] pub mod MapItemRenderer;

#[path = "recipebook/mod.rs"] pub mod recipebook;

#[path = "GuiSubtitleOverlay.rs"] pub mod GuiSubtitleOverlay;
pub mod GuiOverlayDebug;
#[path = "GuiScreenOptionsSounds.rs"] pub mod GuiScreenOptionsSounds;
#[cfg(target_os = "android")]
#[path = "GuiTouchSettings.rs"] pub mod GuiTouchSettings;
#[path = "ScreenChatOptions.rs"] pub mod ScreenChatOptions;
#[path = "GuiCustomizeSkin.rs"] pub mod GuiCustomizeSkin;
#[path = "GuiScreenResourcePacks.rs"] pub mod GuiScreenResourcePacks;

#[path = "GuiAccountManager.rs"] pub mod GuiAccountManager;
#[path = "GuiMicrosoftAuth.rs"] pub mod GuiMicrosoftAuth;
#[path = "GuiSessionLogin.rs"] pub mod GuiSessionLogin;
#[path = "GuiAltCracked.rs"] pub mod GuiAltCracked;

...
use crate::net::minecraft::client::gui::GuiIngame::{HudText, HudTexturedQuad, HudTexture};
...
impl GuiBossOverlay {
    ...
    pub fn read(&mut self, packetIn: &SPacketUpdateBossInfo, systemTimeMillis: u64, locale: &Locale) {
        ...
        match packetIn.getOperation() {
            ...
            _ => {
                if let Some((_, info)) = self.mapBossInfos.iter_mut().find(|(existing, _)| *existing == id) {
    ...
    pub fn buildFrame(
        ...
        for (_, client) in &self.mapBossInfos {
            ...
            let x = guiWidth / 2 - 91;
            append_bar(&mut frame.bars, x, y, info.getColor().ordinal(), info.getOverlay(), 182);
            let filled = (client.getPercent(systemTimeMillis) * 183.0) as i32;
            if filled > 0 {
                append_bar(&mut frame.bars, x, y, info.getColor().ordinal(), info.getOverlay(), filled.min(183));
                // `append_bar` distinguishes fill/background through this
                // marker by replacing the just-added Y coordinate below.
                let count = if info.getOverlay() == Overlay::Progress { 1 } else { 2 };
            ...
            y += 10 + 9;
            if y >= guiHeight / 3 { break; }
        }
        frame
    }

    pub fn clearBossInfos(&mut self) { self.mapBossInfos.clear(); }
    pub fn shouldPlayEndBossMusic(&self) -> bool { self.mapBossInfos.iter().any(|(_, info)| info.info().shouldPlayEndBossMusic()) }
    pub fn shouldDarkenSky(&self) -> bool { self.mapBossInfos.iter().any(|(_, info)| info.info().shouldDarkenSky()) }
    pub fn shouldCreateFog(&self) -> bool { self.mapBossInfos.iter().any(|(_, info)| info.info().shouldCreateFog()) }
}

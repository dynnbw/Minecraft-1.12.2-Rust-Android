...
impl Default for GuiScreenOptionsSounds {
    fn default() -> Self {
        let sliders = SoundCategory::ALL.into_iter().map(|category| {
            (category, GuiOptionSlider::new(100 + category.index() as i32, 0, 0, 1.0, ""))
        }).collect();
        Self {
...
impl GuiScreenOptionsSounds {
    ...
    pub fn initGui(&mut self, width: i32, height: i32, locale: &Locale, settings: &GameSettings) {
        ...
        for (index, (category, slider)) in self.sliders.iter_mut().enumerate() {
            if *category == SoundCategory::Master {
                ...
                let ordinal = index - 1;
                slider.GuiButton.x = if ordinal % 2 == 0 { width / 2 - 155 } else { width / 2 + 5 };
        ...
        self.subtitles.displayString = bool_label(locale, "options.showSubtitles", settings.showSubtitles);
    ...
    fn draw(&mut self, draw: &mut GuiDrawList, font: &mut FontRenderer, mouseX: i32, mouseY: i32, partial: f32, world: bool) {
        ...
        for (_, slider) in &mut self.sliders { slider.drawButton(draw, font, mouseX, mouseY, partial); }
        self.subtitles.drawButton(draw, font, mouseX, mouseY, partial);
        self.done.drawButton(draw, font, mouseX, mouseY, partial);
    }

    pub fn mouseClicked(&mut self, x: i32, y: i32, button: i32, locale: &Locale, settings: &GameSettings) -> Option<GuiScreenOptionsSoundsInteraction> {
        ...
        for (category, slider) in &mut self.sliders {
            if let Some(value) = slider.mousePressed(x, y) {
                ...
                slider.setSliderValue(value);
                slider.setDisplayString(volume_label(locale, *category, value));
                return Some(GuiScreenOptionsSoundsInteraction { action: GuiScreenOptionsSoundsAction::SetSoundLevel(*category, value), sound: None });
            }
        }
        if self.subtitles.mousePressed(x, y) {
            self.subtitles.displayString = bool_label(locale, "options.showSubtitles", !settings.showSubtitles);
            return Some(GuiScreenOptionsSoundsInteraction { action: GuiScreenOptionsSoundsAction::ToggleSubtitles, sound: Some(self.subtitles.playPressSound()) });
        }
        self.done.mousePressed(x, y).then(|| GuiScreenOptionsSoundsInteraction { action: GuiScreenOptionsSoundsAction::Done, sound: Some(self.done.playPressSound()) })
    }

    pub fn mouseDragged(&mut self, x: i32, locale: &Locale) -> Option<GuiScreenOptionsSoundsAction> {
        for (category, slider) in &mut self.sliders {
            if let Some(value) = slider.mouseDragged(x) {
                let value = value.clamp(0.0, 1.0);
                slider.setSliderValue(value);
                slider.setDisplayString(volume_label(locale, *category, value));
                return Some(GuiScreenOptionsSoundsAction::SetSoundLevel(*category, value));
            }
        }
        None
    }
    pub fn mouseReleased(&mut self, x: i32, y: i32) -> Option<GuiSoundCommand> {
        ...
        for (_, slider) in &mut self.sliders { slider.mouseReleased(x, y); }
...
fn volume_label(locale: &Locale, category: SoundCategory, value: f32) -> String {
    ...
    let amount = if value <= 0.0 { locale.translate_key("options.off").to_owned() } else { format!("{}%", (value * 100.0) as i32) };
    format!("{}: {}", locale.translate_key(key), amount)
}
fn bool_label(locale: &Locale, key: &str, value: bool) -> String {
    format!("{}: {}", locale.translate_key(key), locale.translate_key(if value { "options.on" } else { "options.off" }))
}

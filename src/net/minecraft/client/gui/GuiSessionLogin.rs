...
pub enum GuiSessionLoginAction { Cancel, Authenticated(Session), None }
#[derive(Debug, Clone, PartialEq)]
pub struct GuiSessionLoginInteraction { pub action: GuiSessionLoginAction, pub sound: Option<GuiSoundCommand> }

#[derive(Debug)]
enum TokenEvent { Status(String), Success(MicrosoftLogin), Failure(&'static str) }
...
impl GuiSessionLogin {
    ...
    pub fn initGui(&mut self, width: i32, height: i32) {
        ...
        self.GuiScreen.buttonList.push(GuiButton::new(1, width / 2 - 100, height / 2 + 35, "Login"));
        self.GuiScreen.buttonList.push(GuiButton::new(0, width / 2 - 100, height / 2 + 65, "Cancel"));
    }
                ...
                match event {
                    ...
                    TokenEvent::Success(login) => {
                        ...
                        if let Err(error) = config.add(account) {
                            ...
                            self.status = Some(format!("§2Logged in as {}", login.session.getUsername()));
                            self.finishedSession = Some(login.session);
                        }
                        clear = true;
                    }
                    TokenEvent::Failure(tokenType) => { self.status = Some(format!("§4Invalid token ({tokenType})")); clear = true; }
                }
            }
        }
        if clear { self.receiver = None; self.updateButton(); }
        self.finishedSession.take().map(GuiSessionLoginAction::Authenticated)
    }
    ...
    pub fn drawScreen(&mut self, drawList: &mut GuiDrawList, font: &mut FontRenderer, mouseX: i32, mouseY: i32, partialTicks: f32) {
        ...
        if let Some(status) = &self.status {
            font.draw_string_with_shadow(drawList, status, (self.GuiScreen.width / 2 - 100) as f32, (self.GuiScreen.height / 2 - 20) as f32, -1);
        }
        self.GuiScreen.drawScreen(drawList, font, mouseX, mouseY, partialTicks);
    }

    pub fn mouseClicked(&mut self, mouseX: i32, mouseY: i32, mouseButton: i32, font: &FontRenderer) -> Option<GuiSessionLoginInteraction> {
        self.sessionField.mouseClicked(mouseX, mouseY, mouseButton, font);
        ...
        let action = match button.id {
            ...
            1 => { self.startLogin(); GuiSessionLoginAction::None }
            _ => GuiSessionLoginAction::None,
        };
        Some(GuiSessionLoginInteraction { action, sound: Some(button.playPressSound()) })
    }

    pub fn typedText(&mut self, text: &str, font: &FontRenderer) -> bool {
        let changed = self.sessionField.writeText(text, Some(font));
        if changed { self.updateButton(); }
        changed
    }
    ...
    pub fn selectAll(&mut self, font: &FontRenderer) { self.sessionField.selectAll(font); }
    pub fn enterPressed(&mut self) { self.startLogin(); }

    fn startLogin(&mut self) {
        if self.receiver.is_some() { return; }
        let token = self.sessionField.getText().to_owned();
        let tokenType = if token.starts_with("M.C") { "Refresh Token" } else { "Access Token" };
        self.status = Some("§7Logging in...".to_owned());
        let (sender, receiver) = mpsc::channel();
        let statusSender = sender.clone();
        let _ = thread::Builder::new().name("Exhibition Token Login".to_owned()).spawn(move || {
            ...
            let relayThread = thread::spawn(move || {
                while let Ok(status) = uiReceiver.recv() {
                    if relay.send(TokenEvent::Status(status)).is_err() { break; }
                }
            });
            ...
            match result {
                Ok(login) => { let _ = statusSender.send(TokenEvent::Success(login)); }
                Err(_) => { let _ = statusSender.send(TokenEvent::Failure(tokenType)); }
            }
        });
    ...
    fn updateButton(&mut self) {
        ...
        if let Some(button) = self.GuiScreen.buttonList.iter_mut().find(|button| button.id == 1) { button.enabled = enabled; }
    }
}
...
mod tests {
    ...
    fn token_login_buttons_match_exhibition_layout() {
        ...
        let login = screen.GuiScreen.buttonList.iter().find(|button| button.id == 1).unwrap();
        let cancel = screen.GuiScreen.buttonList.iter().find(|button| button.id == 0).unwrap();
        assert_eq!((login.x, login.y, login.displayString.as_str()), (327, 275, "Login"));
        assert_eq!((cancel.x, cancel.y, cancel.displayString.as_str()), (327, 305, "Cancel"));

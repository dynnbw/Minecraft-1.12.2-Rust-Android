...
use crate::net::minecraft::network::PacketBuffer::{PacketCodec, read_var_i32};
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ServerPingFailure { CannotResolve, CannotConnect(String) }
...
impl Default for ServerPinger {
    fn default() -> Self {
        ...
        Self { sender, receiver, pending: HashSet::new(), nextRequestId: 1 }
    }
}
...
impl ServerPinger {
    ...
    pub fn ping(&mut self, serverIndex: usize, serverIP: String) -> u64 {
        ...
        let spawnResult = std::thread::Builder::new().name(format!("Server Pinger #{requestId}")).spawn(move || {
            let result = pingServer(&threadServerIP);
            let _ = sender.send(ServerPingResult { requestId, serverIndex, serverIP: threadServerIP, result });
        });
        if let Err(error) = spawnResult {
            self.pending.remove(&requestId);
            let _ = self.sender.send(ServerPingResult {
                requestId, serverIndex, serverIP,
    ...
    pub fn clearPendingNetworks(&mut self) { self.pending.clear(); }
    pub fn hasPendingNetworks(&self) -> bool { !self.pending.is_empty() }
}
...
fn pingServerModern(serverIP: &str) -> Result<ServerPingSuccess, ServerPingFailure> {
    ...
    if host.is_empty() { return Err(ServerPingFailure::CannotResolve); }
    let socketAddress = (host.as_str(), address.getPort()).to_socket_addrs()
        .map_err(|_| ServerPingFailure::CannotResolve)?
        .next().ok_or(ServerPingFailure::CannotResolve)?;
    ...
    let handshake = C00Handshake::new(host, address.getPort(), ConnectionState::Status).writePacketData()
        .map_err(|error| ServerPingFailure::CannotConnect(error.to_string()))?;
    ...
    if responsePacket.id != 0 { return Err(ServerPingFailure::CannotConnect(format!("unexpected status packet id {}", responsePacket.id))); }
    ...
    writePacket(&mut stream, &codec, &CPacketPing::new(pingSentAt).writePacketData())?;
    let pongPacket = readPacket(&mut stream, &codec)?;
    if pongPacket.id != 1 { return Err(ServerPingFailure::CannotConnect(format!("unexpected pong packet id {}", pongPacket.id))); }
    let pong = SPacketPong::readPacketData(&pongPacket).map_err(|error| ServerPingFailure::CannotConnect(error.to_string()))?;
    if pong.getClientTime() != pingSentAt { return Err(ServerPingFailure::CannotConnect("pong timestamp did not match request".to_owned())); }
    let pingToServer = i64::try_from(pingStarted.elapsed().as_millis()).unwrap_or(i64::MAX);

    let (version, gameVersion) = response.getVersion().map(|value| (value.getProtocol(), value.getName().to_owned()))
        .unwrap_or((0, "Old".to_owned()));
    let (populationInfo, playerList) = if let Some(players) = response.getPlayers() {
        let populationInfo = format!("§7{}§8/§7{}", players.getOnlinePlayerCount(), players.getMaxPlayers());
        let mut names: Vec<String> = players.getPlayers().iter().map(|profile| profile.name.clone()).collect();
        let unlisted = players.getOnlinePlayerCount().saturating_sub(names.len() as i32);
        if unlisted > 0 { names.push(format!("... and {unlisted} more ...")); }
        (populationInfo, if names.is_empty() { None } else { Some(names.join("\n")) })
    } else { ("§8???".to_owned(), None) };
    let serverIcon = response.getFavicon().and_then(|value| value.strip_prefix("data:image/png;base64,")).map(str::to_owned);

    Ok(ServerPingSuccess {
        serverMOTD: response.getServerDescription().unwrap_or_default().to_owned(), populationInfo, pingToServer,
        version, gameVersion, playerList, serverIcon,
    })
}
...
fn tryCompatibilityPing(serverIP: &str) -> Result<ServerPingSuccess, ServerPingFailure> {
    ...
    stream.write_u16::<BigEndian>(u16::try_from(payloadLength).map_err(|_| ServerPingFailure::CannotConnect("legacy ping host is too long".to_owned()))?).map_err(mapIo)?;
    stream.write_u8(127).map_err(mapIo)?;
    stream.write_u16::<BigEndian>(u16::try_from(hostUnits.len()).map_err(|_| ServerPingFailure::CannotConnect("legacy ping host is too long".to_owned()))?).map_err(mapIo)?;
    for unit in hostUnits { stream.write_u16::<BigEndian>(unit).map_err(mapIo)?; }
    stream.write_i32::<BigEndian>(i32::from(address.getPort())).map_err(mapIo)?;
    stream.flush().map_err(mapIo)?;

    if stream.read_u8().map_err(mapIo)? != 0xFF {
        return Err(ServerPingFailure::CannotConnect("legacy ping returned an unexpected packet".to_owned()));
    }
    let charCount = stream.read_u16::<BigEndian>().map_err(mapIo)? as usize;
    let mut units = Vec::with_capacity(charCount);
    for _ in 0..charCount { units.push(stream.read_u16::<BigEndian>().map_err(mapIo)?); }
    let response = String::from_utf16(&units)
        .map_err(|_| ServerPingFailure::CannotConnect("legacy ping returned invalid UTF-16".to_owned()))?;
    let fields: Vec<&str> = response.splitn(6, '\0').collect();
    if fields.len() != 6 || fields[0] != "§1" {
        return Err(ServerPingFailure::CannotConnect("legacy ping returned an invalid response".to_owned()));
    }
...
fn writeUtf16Be(output: &mut impl Write, value: &str) -> io::Result<()> {
    ...
    output.write_u16::<BigEndian>(u16::try_from(units.len()).map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "UTF-16 string is too long"))?)?;
    for unit in units { output.write_u16::<BigEndian>(unit)?; }
    Ok(())
}

fn writePacket(stream: &mut TcpStream, codec: &PacketCodec, packet: &RawPacket) -> Result<(), ServerPingFailure> {
    let encoded = codec.encode(packet).map_err(|error| ServerPingFailure::CannotConnect(error.to_string()))?;
    stream.write_all(&encoded).map_err(|error| ServerPingFailure::CannotConnect(error.to_string()))
}
...
fn readPacket(stream: &mut TcpStream, codec: &PacketCodec) -> Result<RawPacket, ServerPingFailure> {
    ...
    if packetLength < 0 || packetLength as usize > 2 * 1024 * 1024 {
        return Err(ServerPingFailure::CannotConnect(format!("invalid status packet length {packetLength}")));
    }
    ...
    codec.decode(&mut input).map_err(|error| ServerPingFailure::CannotConnect(error.to_string()))
}
    ...
    for _ in 0..5 {
        ...
        if byte[0] & 0x80 == 0 {
            let value = {
                ...
                read_var_i32(&mut view).map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))?
            };
            return Ok((value, bytes));
        }
    }
    Err(io::Error::new(io::ErrorKind::InvalidData, "VarInt is too large"))
}

fn mapIo(error: io::Error) -> ServerPingFailure {
    if matches!(error.kind(), io::ErrorKind::NotFound | io::ErrorKind::AddrNotAvailable) { ServerPingFailure::CannotResolve }
    else { ServerPingFailure::CannotConnect(error.to_string()) }
}
...
mod tests {
    ...
    fn status_failure_categories_are_stable() {
        assert!(matches!(pingServer("[invalid"), Err(ServerPingFailure::CannotResolve) | Err(ServerPingFailure::CannotConnect(_))));
    }

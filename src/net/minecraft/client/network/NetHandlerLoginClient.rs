...
use crate::com::mojang::authlib::minecraft::MinecraftSessionService::{JoinServerError, MinecraftSessionService};
...
pub enum NetHandlerLoginClientError {
    #[error(transparent)] Network(#[from] NetworkManagerError),
    #[error("invalid encryption request: {0}")] EncryptionRequest(String),
    #[error("invalid login success packet: {0}")] LoginSuccess(String),
    #[error("invalid compression packet: {0}")] Compression(String),
    #[error("invalid disconnect packet: {0}")] Disconnect(String),
    #[error("login authentication failed: {0}")] Authentication(String),
}
...
impl NetHandlerLoginClient {
    pub fn new(session: Session, isLanServer: bool) -> Self {
        Self { session, isLanServer, sessionService: MinecraftSessionService::new() }
    }
    ...
    pub fn processPacket(
        ...
        match packet.id {
            ...
            3 => {
                ...
                Ok(LoginHandlerEvent::CompressionEnabled(packet.getCompressionThreshold()))
            }
            id => Err(NetHandlerLoginClientError::LoginSuccess(format!("unexpected login packet id {id}"))),
        }
    }
    ...
    fn handleEncryptionRequest(
        ...
        let serverHash = getServerIdHashHex(packet.getServerId(), packet.getPublicKeyEncoded(), &secretKey);
        let profile = self.session.getProfile();
        if let Err(error) = self.sessionService.joinServer(&profile, self.session.getToken(), &serverHash) {
            if self.isLanServer {
                ...
                return Err(NetHandlerLoginClientError::Authentication(authentication_message(error)));
            }
        }
        let response = CPacketEncryptionResponse::new(&secretKey, packet.getPublicKey(), packet.getVerifyToken())
            .map_err(|error| NetHandlerLoginClientError::EncryptionRequest(error.to_string()))?
            .writePacketData()
...
fn authentication_message(error: JoinServerError) -> String {
    match error {
        JoinServerError::AuthenticationUnavailable(_) => "disconnect.loginFailedInfo.serversUnavailable".to_owned(),
        JoinServerError::InvalidCredentials(_) => "disconnect.loginFailedInfo.invalidSession".to_owned(),

...
impl RenderRabbit {
    pub fn supports(entityType: MobEntityType) -> bool { entityType.registryName == "rabbit" }
    pub fn texture(entity: &EntityOtherClient) -> ResourceLocation {
        ...
        let path = if toast { "textures/entity/rabbit/toast.png" } else { match entity.rabbitType() {
...
fn strip_formatting(value: &str) -> String {
    let mut out=String::new(); let mut skip=false;
    for c in value.chars() { if skip { skip=false; continue; } if c=='§' { skip=true; } else { out.push(c); } }
    out
}

...
impl RenderShulker {
    ...
    pub fn allTextures() -> Vec<ResourceLocation> {
        ...
        .into_iter()
        .map(|name| ResourceLocation::new("minecraft", format!("textures/entity/shulker/shulker_{name}.png")))
        .collect()
    }
    ...
    pub fn teleportRenderOffset(entity: &EntityOtherClient, partialTicks: f32) -> [f32; 3] {
        let Some(current) = entity.shulkerAttachmentPos() else { return [0.0; 3]; };
        let Some(old) = entity.shulkerOldAttachPos() else { return [0.0; 3]; };
        let ticks = entity.shulkerClientTeleportInterp();
        if ticks <= 0 || !entity.shulkerIsAttachedToBlock() { return [0.0; 3]; }
    ...
    pub fn corpseTransform(facing: EnumFacing) -> &'static [ShulkerTransformOp] {
        ...
        match facing {
            ...
            EnumFacing::East => &[
                ...
                Rotate { degrees: 90.0, axis: [1.0, 0.0, 0.0] },
                Rotate { degrees: 90.0, axis: [0.0, 0.0, 1.0] },
            ],
            EnumFacing::West => &[
                Translate([-0.5, 0.5, 0.0]),
                Rotate { degrees: 90.0, axis: [1.0, 0.0, 0.0] },
            ...
            EnumFacing::South => &[
                Translate([0.0, 0.5, 0.5]),
                Rotate { degrees: 90.0, axis: [1.0, 0.0, 0.0] },
                Rotate { degrees: 180.0, axis: [0.0, 0.0, 1.0] },
            ],
            EnumFacing::Up => &[
                Translate([0.0, 1.0, 0.0]),
                Rotate { degrees: 180.0, axis: [1.0, 0.0, 0.0] },
            ],
        }
    }
    ...
    pub fn headLayerTransform(facing: EnumFacing) -> &'static [ShulkerTransformOp] {
        ...
        match facing {
            ...
            EnumFacing::East => &[
                ...
                Rotate { degrees: 180.0, axis: [0.0, 1.0, 0.0] },
            ],
            EnumFacing::West => &[
                Rotate { degrees: -90.0, axis: [0.0, 0.0, 1.0] },
                Rotate { degrees: 90.0, axis: [1.0, 0.0, 0.0] },
...
mod tests {
    ...
    fn exposes_all_sixteen_dye_textures_in_metadata_order() {
        ...
        assert_eq!(textures[0].getPath(), "textures/entity/shulker/shulker_white.png");
        assert_eq!(textures[10].getPath(), "textures/entity/shulker/shulker_purple.png");
        assert_eq!(textures[15].getPath(), "textures/entity/shulker/shulker_black.png");
    }
}

...
impl RenderIllusionIllager {
    pub fn supports(entityType: MobEntityType) -> bool { entityType.registryName == "illusion_illager" }
    pub fn texture() -> ResourceLocation { ResourceLocation::new("minecraft", "textures/entity/illager/illusionist.png") }
    pub const fn preScale() -> f32 { 0.9375 }
    pub fn armPose(entity: &EntityOtherClient) -> IllagerArmPose {
        if entity.illagerSpellcasting() { IllagerArmPose::Spellcasting }
        else if entity.illagerAttacking() { IllagerArmPose::BowAndArrow }
        else { IllagerArmPose::Crossed }
    }
    pub fn shouldRenderHeldItem(entity: &EntityOtherClient) -> bool {
        entity.illagerSpellcasting() || entity.illagerAttacking()
    }
    pub const fn showHood() -> bool { true }

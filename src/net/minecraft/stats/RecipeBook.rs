...
impl RecipeBook {
    pub fn copyFrom(&mut self, other: &Self) {
        ...
        self.recipesToBeDisplayed.clone_from(&other.recipesToBeDisplayed);
    }
    pub fn unlock(&mut self, recipe: IRecipe) {
        if !recipe.isDynamic() { self.knownRecipes.insert(recipe.getId()); }
    }
    pub fn unlockById(&mut self, recipeId: i32) -> bool {
        let Some(recipe) = CraftingManager::getRecipe(recipeId) else { return false; };
        self.unlock(recipe);
        true
    }
    pub fn isUnlocked(&self, recipe: IRecipe) -> bool { self.knownRecipes.contains(&recipe.getId()) }
    pub fn isUnlockedById(&self, recipeId: i32) -> bool { self.knownRecipes.contains(&recipeId) }
    ...
    pub fn isNew(&self, recipe: IRecipe) -> bool { self.recipesToBeDisplayed.contains(&recipe.getId()) }
    pub fn markSeen(&mut self, recipe: IRecipe) { self.recipesToBeDisplayed.remove(&recipe.getId()); }
    pub fn markNew(&mut self, recipe: IRecipe) { self.recipesToBeDisplayed.insert(recipe.getId()); }
    ...
    pub fn setGuiOpen(&mut self, value: bool) { self.guiOpen = value; }
    pub const fn isFilteringCraftable(&self) -> bool { self.filteringCraftable }
    pub fn setFilteringCraftable(&mut self, value: bool) { self.filteringCraftable = value; }
    pub fn knownRecipeIds(&self) -> impl Iterator<Item = i32> + '_ { self.knownRecipes.iter().copied() }
    pub fn newRecipeIds(&self) -> impl Iterator<Item = i32> + '_ { self.recipesToBeDisplayed.iter().copied() }
}

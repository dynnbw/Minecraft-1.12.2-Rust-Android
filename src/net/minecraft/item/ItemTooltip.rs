...
pub fn getTooltip(stack: &ItemStack, locale: &Locale, advanced: bool) -> Vec<String> {
    ...
    let hide_flags = stack.tagCompound.as_ref().map(|tag| tag.getInteger("HideFlags")).unwrap_or(0);
    if let Some(root) = &stack.tagCompound {
        if hide_flags & 1 == 0 {
            appendEnchantments(&mut lines, root.getTagList("ench", TAG_COMPOUND), locale);
            if stack.itemId == 403 {
                appendEnchantments(&mut lines, root.getTagList("StoredEnchantments", TAG_COMPOUND), locale);
            }
        }
        if root.hasKeyWithType("display", TAG_COMPOUND) {
            ...
            if display.hasKey("color") {
                if advanced {
                    lines.push(formatTranslation(locale, "item.color", &[format!("#{:06X}", display.getInteger("color") & 0xFF_FFFF)]));
                } else {
        ...
        appendBlockList(&mut lines, root, "CanDestroy", "item.canBreak", locale, hide_flags & 8 == 0);
        appendBlockList(&mut lines, root, "CanPlaceOn", "item.canPlace", locale, hide_flags & 16 == 0);
    }
    if advanced {
        if stack.isItemDamaged() {
            lines.push(formatTranslation(locale, "item.durability", &[
                (stack.getMaxDamage() - stack.itemDamage as i32).to_string(),
                stack.getMaxDamage().to_string(),
            ]));
        }
        lines.push(format!("§8{}", ItemRegistryData::definition(stack.itemId).registryName));
        if let Some(root) = &stack.tagCompound {
            lines.push(format!("§8{}", formatTranslation(locale, "item.nbt_tags", &[root.getKeySet().count().to_string()])));
        }
    }
    lines
}
...
pub fn getItemToolTip(stack: &ItemStack, locale: &Locale, advanced: bool) -> Vec<String> {
    ...
    for (index, line) in lines.iter_mut().enumerate() {
        let prefix = if index == 0 { rarityFormatting(stack) } else { "§7" };
...
pub fn localizedName(stack: &ItemStack, locale: &Locale) -> String {
    ...
    if stack.itemId == 442 {
        if let Some(blockEntityTag) = stack.tagCompound.as_ref()
    ...
    ItemTranslationKeys::translationKey(stack.itemId, stack.itemDamage)
        ...
        .unwrap_or_else(|| ItemRegistryData::definition(stack.itemId).registryName.to_owned())
}
...
fn rarityFormatting(stack: &ItemStack) -> &'static str {
    ...
    if stack.itemId == 322 { return if stack.itemDamage == 0 { "§b" } else { "§d" }; }
    if (2256..=2267).contains(&stack.itemId) { return "§b"; }
    if stack.itemId == 403 {
        let stored = stack.tagCompound.as_ref()
            .map(|root| root.getTagList("StoredEnchantments", TAG_COMPOUND))
            .is_some_and(|list| !list.hasNoTags());
        if stored { return "§e"; }
    }
    if stack.isItemEnchanted() { "§b" } else { "§f" }
}

fn customDisplayName(stack: &ItemStack) -> Option<String> {
    let root = stack.tagCompound.as_ref()?;
    if !root.hasKeyWithType("display", TAG_COMPOUND) { return None; }
...
fn appendEnchantments(lines: &mut Vec<String>, list: crate::net::minecraft::nbt::NBTTagList::NBTTagList, locale: &Locale) {
    for index in 0..list.tagCount() {
        ...
        let Some(key) = enchantmentKey(id) else { continue; };
...
fn appendBlockList(lines: &mut Vec<String>, root: &crate::net::minecraft::nbt::NBTTagCompound::NBTTagCompound, tag: &str, title: &str, locale: &Locale, visible: bool) {
    if !visible { return; }
...
fn formatTranslation(locale: &Locale, key: &str, values: &[String]) -> String {
    ...
    for (index, value) in values.iter().enumerate() {
        ...
        if text.contains(&positional) { text = text.replace(&positional, value); }
        else if text.contains("%s") { text = text.replacen("%s", value, 1); }
    }
    text
}
...
fn enchantmentKey(id: i16) -> Option<&'static str> {
    Some(match id {
        0 => "enchantment.protect.all", 1 => "enchantment.protect.fire",
        2 => "enchantment.protect.fall", 3 => "enchantment.protect.explosion",
        4 => "enchantment.protect.projectile", 5 => "enchantment.oxygen",
        6 => "enchantment.waterWorker", 7 => "enchantment.thorns",
        8 => "enchantment.waterWalker", 9 => "enchantment.frostWalker",
        10 => "enchantment.binding_curse", 16 => "enchantment.damage.all",
        17 => "enchantment.damage.undead", 18 => "enchantment.damage.arthropods",
        19 => "enchantment.knockback", 20 => "enchantment.fire",
        21 => "enchantment.lootBonus", 22 => "enchantment.sweeping",
        32 => "enchantment.digging", 33 => "enchantment.untouching",
        34 => "enchantment.durability", 35 => "enchantment.lootBonusDigger",
        48 => "enchantment.arrowDamage", 49 => "enchantment.arrowKnockback",
        50 => "enchantment.arrowFire", 51 => "enchantment.arrowInfinite",
        61 => "enchantment.lootBonusFishing", 62 => "enchantment.fishingSpeed",
        70 => "enchantment.mending", 71 => "enchantment.vanishing_curse",
        _ => return None,
    })
}
...
mod tests {
    ...
    fn custom_name_and_lore_keep_vanilla_format_codes() {
        ...
        let stack = ItemStack { itemId: 70, count: 1, itemDamage: 0, tagCompound: Some(root) };
    ...
    fn gui_tooltip_applies_mcp_rarity_and_secondary_gray() {
        let stack = ItemStack { itemId: 322, count: 1, itemDamage: 1, tagCompound: None };
    ...
    fn banner_display_name_uses_dye_damage_not_metadata_order() {
        ...
        let stack = ItemStack { itemId: 425, count: 1, itemDamage: 1, tagCompound: None };

...
impl Ingredient {
    ...
    pub fn fromStacks(stacks: Vec<ItemStack>) -> Self { Self { matchingStacks: stacks } }
    pub fn fromEncodedAlternatives(encoded: &str) -> Self {
        if encoded.is_empty() { return Self::empty(); }
        let stacks = encoded.split('|').filter_map(|value| {
            let mut parts = value.split(':');
            let itemId = parts.next()?.parse::<i16>().ok()?;
            let itemDamage = parts.next()?.parse::<i16>().ok()?;
            let count = parts.next()?.parse::<u8>().ok()?;
            if parts.next().is_some() { return None; }
            Some(ItemStack { itemId, count, itemDamage, tagCompound: None })
        }).collect();
        Self::fromStacks(stacks)
    }
    pub fn apply(&self, stack: &ItemStack) -> bool {
        if self.matchingStacks.is_empty() { return stack.isEmpty(); }
    ...
    pub fn getMatchingStacks(&self) -> &[ItemStack] { &self.matchingStacks }
}
...
mod tests {
    ...
    fn wildcard_metadata_and_empty_slot_follow_mcp() {
        ...
        assert!(coal.apply(&ItemStack { itemId: 263, count: 2, itemDamage: 1, tagCompound: None }));
        let wildcard = Ingredient::fromEncodedAlternatives("5:32767:1");
        assert!(wildcard.apply(&ItemStack { itemId: 5, count: 1, itemDamage: 4, tagCompound: None }));

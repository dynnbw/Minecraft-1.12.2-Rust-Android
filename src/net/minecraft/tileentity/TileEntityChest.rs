...
impl TileEntityChest {
    pub fn new(pos: BlockPos) -> Self {
        Self { pos, lidAngle: 0.0, prevLidAngle: 0.0, numPlayersUsing: 0 }
    }

    pub fn fromNbt(tag: &NBTTagCompound) -> Option<Self> {
        let id = tag.getString("id");
        if !matches!(id.as_str(), "minecraft:chest" | "Chest" | "minecraft:trapped_chest") {
            return None;
        }
    ...
    pub fn update(&mut self) {
        ...
        {
            if self.numPlayersUsing > 0 { self.lidAngle += 0.1; }
            else { self.lidAngle -= 0.1; }
            self.lidAngle = self.lidAngle.clamp(0.0, 1.0);
        }
    }

    pub fn interpolatedLidAngle(&self, partialTicks: f32) -> f32 {
        let angle = self.prevLidAngle + (self.lidAngle - self.prevLidAngle) * partialTicks.clamp(0.0, 1.0);

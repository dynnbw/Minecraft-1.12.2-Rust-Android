...
impl TileEntityPiston {
    pub fn fromNbt(tag: &NBTTagCompound) -> Option<Self> {
        ...
        if !id.is_empty() && id != "minecraft:piston" && id != "Piston" { return None; }
    ...
    pub fn update(&mut self) {
        ...
        if self.lastProgress < 1.0 { self.progress = (self.progress + 0.5).min(1.0); }
    }

    pub fn nextProgress(&self) -> f32 {
        if self.progress >= 1.0 { 1.0 } else { (self.progress + 0.5).min(1.0) }
    }
    ...
    pub fn finished(&self) -> bool { self.progress >= 1.0 && self.lastProgress >= 1.0 }

    pub fn movementDirection(&self) -> EnumFacing {
        if self.extending { self.pistonFacing } else { self.pistonFacing.opposite() }
    }

    pub fn isMovingSlimeBlock(&self) -> bool { self.pistonState.getBlockId() == 165 }
    ...
    pub fn collisionBoxesLocal(&self) -> Vec<AxisAlignedBB> {
        ...
        if !self.extending && self.shouldHeadBeRendered {
            ...
            boxes.push(BlockPistonBase::getBoundingBox(IBlockState::fromGlobalStateId(
                (self.pistonState.getBlockId() << 4) | metadata,
            )));
        }
        ...
        if self.progress >= 1.0 || push_direction != Some(self.movementDirection()) {
            ...
            boxes.extend(queryBoxes.into_iter().map(|bounds| {
                bounds.offset(offset.0, offset.1, offset.2)
            }));
        }
        boxes
    }
    ...
    pub fn sweptEntityBounds(&self, next_progress: f32) -> Option<AxisAlignedBB> {
        ...
        for bounds in &boxes { union = union.union(*bounds); }
    ...
    pub fn primaryPushDistance(&self, entity_box: AxisAlignedBB, next_progress: f32) -> f64 {
        ...
        if distance > 0.0 { distance.min(delta) + 0.01 } else { 0.0 }
    }
    ...
    pub fn retractionCorrectionDistance(&self, entity_box: AxisAlignedBB, delta: f64) -> f64 {
        if self.extending || !self.shouldHeadBeRendered { return 0.0; }
        let full = AxisAlignedBB::from_block(self.pos);
        if !entity_box.intersects(full) { return 0.0; }
        ...
        if (direct - clipped).abs() < 0.01 { direct.min(delta) + 0.01 } else { 0.0 }
    }
    ...
    fn movingShapeBoxes(&self) -> Vec<AxisAlignedBB> {
        if !self.extending && self.shouldHeadBeRendered {
            ...
            self.pistonState.getBlock().getCollisionBoxes(self.pistonState)
        }
    }
    ...
    fn sweepStrip(bounds: AxisAlignedBB, direction: EnumFacing, delta: f64) -> AxisAlignedBB {
        ...
        match direction {
            EnumFacing::West => AxisAlignedBB::new(bounds.min_x + low, bounds.min_y, bounds.min_z, bounds.min_x + high, bounds.max_y, bounds.max_z),
            EnumFacing::East => AxisAlignedBB::new(bounds.max_x + low, bounds.min_y, bounds.min_z, bounds.max_x + high, bounds.max_y, bounds.max_z),
            EnumFacing::Down => AxisAlignedBB::new(bounds.min_x, bounds.min_y + low, bounds.min_z, bounds.max_x, bounds.min_y + high, bounds.max_z),
            EnumFacing::Up => AxisAlignedBB::new(bounds.min_x, bounds.max_y + low, bounds.min_z, bounds.max_x, bounds.max_y + high, bounds.max_z),
            EnumFacing::North => AxisAlignedBB::new(bounds.min_x, bounds.min_y, bounds.min_z + low, bounds.max_x, bounds.max_y, bounds.min_z + high),
            EnumFacing::South => AxisAlignedBB::new(bounds.min_x, bounds.min_y, bounds.max_z + low, bounds.max_x, bounds.max_y, bounds.max_z + high),
        }
    }
...
mod tests {
    ...
    fn piston_head_has_plate_and_arm() {
        assert_eq!(BlockPistonExtension::collisionBoxesForFacing(EnumFacing::Up, false).len(), 2);
    }
}

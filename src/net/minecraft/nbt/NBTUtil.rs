...
pub fn areNBTEquals(expected: &NBTTagCompound, candidate: &NBTTagCompound, compareListTag: bool) -> bool {
    expected.getKeySet().all(|key| match (expected.getTag(key), candidate.getTag(key)) {
...
fn areTagEquals(expected: &NBTBase, candidate: &NBTBase, compareListTag: bool) -> bool {
    match (expected, candidate) {
        (NBTBase::Compound(left), NBTBase::Compound(right)) => areNBTEquals(left, right, compareListTag),
        (NBTBase::List(left), NBTBase::List(right)) if !compareListTag => {
            if left.tagCount() == 0 { return right.tagCount() == 0; }
            if left.tagCount() > right.tagCount() { return false; }
            (0..left.tagCount()).all(|i| {
                let Some(wanted) = left.tags().get(i) else { return false; };
                (0..right.tagCount()).any(|j| right.tags().get(j).is_some_and(|got| areTagEquals(wanted, got, false)))
            })
        }
...
mod tests {
    ...
    fn expected_compound_is_a_subset() {
        let mut expected = NBTTagCompound::new(); expected.setInteger("a", 1);
        let mut candidate = expected.clone(); candidate.setString("extra", "allowed");

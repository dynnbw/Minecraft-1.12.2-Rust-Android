...
impl NBTTagCompound {
    pub fn new() -> Self { Self { tagMap: emptyCompoundMap() } }
    pub const fn getId(&self) -> u8 { TAG_COMPOUND }

    pub fn setTag(&mut self, key: impl Into<String>, value: NBTBase) { self.tagMap.insert(key.into(), value); }
    pub fn setByte(&mut self, key: impl Into<String>, value: i8) { self.setTag(key, NBTBase::Byte(value)); }
    pub fn setBoolean(&mut self, key: impl Into<String>, value: bool) { self.setByte(key, if value { 1 } else { 0 }); }
    ...
    pub fn setByteArray(&mut self, key: impl Into<String>, value: Vec<u8>) { self.setTag(key, NBTBase::ByteArray(value)); }
    pub fn setIntArray(&mut self, key: impl Into<String>, value: Vec<i32>) { self.setTag(key, NBTBase::IntArray(value)); }
    pub fn setLongArray(&mut self, key: impl Into<String>, value: Vec<i64>) { self.setTag(key, NBTBase::LongArray(value)); }
    pub fn setCompoundTag(&mut self, key: impl Into<String>, value: NBTTagCompound) { self.setTag(key, NBTBase::Compound(value)); }
    pub fn setTagList(&mut self, key: impl Into<String>, value: NBTTagList) { self.setTag(key, NBTBase::List(value)); }

    pub fn getTag(&self, key: &str) -> Option<&NBTBase> { self.tagMap.get(key) }
    pub fn getTagId(&self, key: &str) -> u8 { self.getTag(key).map(NBTBase::getId).unwrap_or(TAG_END) }
    pub fn hasKey(&self, key: &str) -> bool { self.tagMap.contains_key(key) }
    ...
    pub fn getCompoundTag(&self, key: &str) -> NBTTagCompound { match self.getTag(key) { Some(NBTBase::Compound(v)) => v.clone(), _ => Self::new() } }
    ...
    pub fn getKeySet(&self) -> impl Iterator<Item = &String> { self.tagMap.keys() }
    pub fn removeTag(&mut self, key: &str) { self.tagMap.remove(key); }
    pub fn hasNoTags(&self) -> bool { self.tagMap.is_empty() }
    ...
    pub(crate) fn read<R: Read>(input: &mut R, depth: usize) -> io::Result<Self> {
        if depth > 512 { return Err(io::Error::new(io::ErrorKind::InvalidData, "Tried to read NBT tag with too high complexity, depth > 512")); }

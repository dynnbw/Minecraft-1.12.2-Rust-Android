...
/// Numeric registry order from MCP 1.12.2 `SoundEvent#registerSounds`.
...
mod tests {
    ...
    fn registry_is_bidirectional() {
        for id in [0, 100, SOUND_EVENT_NAMES.len() as i32 - 1] {
            let location = SoundEvent::getById(id).unwrap();
            assert_eq!(SoundEvent::getId(&location), Some(id));
        }
    }
}

...
impl InventoryMerchant {
    pub fn getCurrentRecipe(&self) -> Option<&MerchantRecipe> { self.currentRecipe.as_ref() }
    pub const fn getCurrentRecipeIndex(&self) -> i32 { self.currentRecipeIndex }
    pub fn setCurrentRecipeIndex(&mut self, index: i32) { self.currentRecipeIndex = index.max(0); }
    pub fn getRecipes(&self) -> Option<&MerchantRecipeList> { self.recipes.as_ref() }
    pub fn setRecipes(&mut self, recipes: MerchantRecipeList) { self.recipes=Some(recipes); }
    pub fn resetRecipeAndSlots(&mut self, first: &ItemStack, second: &ItemStack) -> ItemStack {
        self.currentRecipe=None;
        let (primary, secondary)=if first.isEmpty() && !second.isEmpty() {(second,first)} else {(first,second)};
        let found=self.recipes.as_ref().and_then(|list| {
        ...
        }).filter(|recipe| !recipe.isRecipeDisabled()).cloned();
        self.currentRecipe=found.clone();
        found.map(|r| r.getItemToSell().clone()).unwrap_or(ItemStack::EMPTY)
    }
}

...
pub struct ContainerMerchant { state: ContainerWindow, merchantInventory: InventoryMerchant }
impl ContainerMerchant {
    pub fn new(windowId:i32,title:ITextComponent,reportedSlotCount:usize,playerInventory:&InventoryPlayer)->Result<Self,CodecError>{
        ContainerWindow::new(windowId,title,reportedSlotCount,playerInventory,ContainerWindowKind::Merchant)
            .map(|state|Self{state,merchantInventory:InventoryMerchant::default()})
    }
    pub const fn state(&self)->&ContainerWindow{&self.state}
    pub fn stateMut(&mut self)->&mut ContainerWindow{&mut self.state}
    pub fn getRecipes(&self)->Option<&MerchantRecipeList>{self.merchantInventory.getRecipes()}
    pub fn setRecipes(&mut self,recipes:MerchantRecipeList){self.merchantInventory.setRecipes(recipes);self.resetRecipeAndSlots();}
    pub const fn getCurrentRecipeIndex(&self)->i32{self.merchantInventory.getCurrentRecipeIndex()}
    pub fn setCurrentRecipeIndex(&mut self,index:i32){self.merchantInventory.setCurrentRecipeIndex(index);self.resetRecipeAndSlots();}

...
impl ColorizerFoliage {
    ...
    pub fn getFoliageColor(&self, temperature: f64, humidity: f64) -> i32 {
        ...
        let index = (j << 8 | i).clamp(0, self.foliageBuffer.len().saturating_sub(1) as i32) as usize;
        self.foliageBuffer.get(index).copied().unwrap_or(Self::getFoliageColorBasic())
    }

    pub const fn getFoliageColorPine() -> i32 { 6_396_257 }
    pub const fn getFoliageColorBirch() -> i32 { 8_431_445 }
    pub const fn getFoliageColorBasic() -> i32 { 4_764_952 }
}

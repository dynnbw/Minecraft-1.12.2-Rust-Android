...
use crate::net::minecraft::network::PacketBuffer::{read_bool, read_i32_be, read_u8, write_bool, write_i32_be, CodecError};
...
impl MerchantRecipeList {
    ...
    pub fn isEmpty(&self) -> bool { self.0.is_empty() }
    pub fn get(&self, index: usize) -> Option<&MerchantRecipe> { self.0.get(index) }
    pub fn iter(&self) -> impl Iterator<Item=&MerchantRecipe> { self.0.iter() }
    pub fn push(&mut self, recipe: MerchantRecipe) { self.0.push(recipe); }
    ...
    fn areItemStacksExactlyEqual(stack1: &ItemStack, stack2: &ItemStack) -> bool {
        ItemStack::areItemsEqual(stack1, stack2)
            && match &stack2.tagCompound {
                None => true,
                Some(expected) => stack1.tagCompound.as_ref().is_some_and(|candidate| areNBTEquals(expected, candidate, false)),
            }
    }
    pub fn canRecipeBeUsed(&self, first: &ItemStack, second: &ItemStack, selected: i32) -> Option<&MerchantRecipe> {
    ...
    pub fn writeToBuf(&self, output: &mut Vec<u8>) -> Result<(), CodecError> {
        if self.0.len() > u8::MAX as usize { return Err(CodecError::InvalidData("more than 255 merchant recipes".to_owned())); }
        ...
        for recipe in &self.0 {
            ...
            if recipe.hasSecondItemToBuy() { recipe.getSecondItemToBuy().writeToBuffer(output)?; }
    ...
    pub fn readFromBuf(input: &mut &[u8]) -> Result<Self, CodecError> {
        ...
        for _ in 0..count {
            ...
            let second = if read_bool(input)? { ItemStack::readFromBuffer(input)? } else { ItemStack::EMPTY };
            ...
            if disabled { recipe.compensateToolUses(); }
            result.push(recipe);
        }
        Ok(result)
    }
...
mod tests {
    use super::*;
    fn stack(id:i16,count:u8)->ItemStack { ItemStack{itemId:id,count,itemDamage:0,tagCompound:None} }
    #[test] fn packet_round_trip_and_selected_zero_scan() {
        let mut list=MerchantRecipeList::new(); list.push(MerchantRecipe::new(stack(388,3),ItemStack::EMPTY,stack(297,1),2,7));
        let mut bytes=Vec::new(); list.writeToBuf(&mut bytes).unwrap(); let mut input=bytes.as_slice(); let decoded=MerchantRecipeList::readFromBuf(&mut input).unwrap();
        assert!(input.is_empty()); assert!(decoded.canRecipeBeUsed(&stack(388,3),&ItemStack::EMPTY,0).is_some());
    }
}

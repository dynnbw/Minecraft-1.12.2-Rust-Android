...
pub unsafe fn initialize_android_context(java_vm: *mut c_void, context_jobject: *mut c_void) {
    // Patched: Android 15+ can re-run activity onCreate inside a live
    // process (activity recreation), and android-activity 0.6.1 initializes
    // ndk-context on every onCreate. Upstream asserts the once-only
    // contract and aborts; keep the newest context instead so a recreated
    // activity resumes without crashing the process.

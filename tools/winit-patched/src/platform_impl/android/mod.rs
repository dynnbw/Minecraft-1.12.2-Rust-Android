        ...
        F: FnMut(event::Event<T>, &RootAEL),
    {
        ...
        match event {
            InputEvent::MotionEvent(motion_event) => {
                ...
                // Android physical mouse support: upstream winit leaves mouse
                ...
                if is_mouse {
                    ...
                    let mouse_button = || {
                        if motion_event.button_state().secondary() {
                            event::MouseButton::Right
                        } else if motion_event.button_state().teriary() {
                    ...
                    match motion_event.action() {
                        ...
                        MotionAction::Scroll => {
                            let delta = event::MouseScrollDelta::LineDelta(
                                pointer.axis_value(Axis::Hscroll) as f32,
                                pointer.axis_value(Axis::Vscroll) as f32,
                            );
                            let event = event::Event::WindowEvent {
                                window_id,
